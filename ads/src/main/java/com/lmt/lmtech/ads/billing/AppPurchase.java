...
import com.lmt.lmtech.ads.funtion.BillingListener;
import com.lmt.lmtech.ads.funtion.PurchaseListener;
import com.lmt.lmtech.ads.funtion.UpdatePurchaseListener;
...
public class AppPurchase {
    ...
    public double getPriceWithoutCurrencySubscription(String productId) {
    ...
    private String getFreeTrialPeriod(ProductDetails productDetails) {
        if (productDetails.getProductType().equals(BillingClient.ProductType.SUBS)) {
            ...
            for (ProductDetails.SubscriptionOfferDetails offer : offers) {
                for (ProductDetails.PricingPhase price : offer.getPricingPhases().getPricingPhaseList()) {
                    if (price.getPriceAmountMicros() == 0L) {
                        return price.getBillingPeriod();
                    }
                }
            }
        }
    ...
    public int getFreeTrialDays(String productId) {
        ProductDetails skuDetails = skuDetailsSubsMap.get(productId);
        if (skuDetails != null) {
            String period = getFreeTrialPeriod(skuDetails);
            if (!TextUtils.isEmpty(period)) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                    return java.time.Period.parse(period).getDays();
                } else {
                    return org.threeten.bp.Period.parse(period).getDays();
                }
            } else {
                return 0;
            }
     ...
     * @return
     */
    public double getPriceWithoutCurrency(String productId) {
        ProductDetails skuDetails = skuDetailsSubsMap.get(productId);
        if (skuDetails == null) {
            return 0;
        }
