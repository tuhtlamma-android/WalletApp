package com.lmt.lmtech.ads.admob;
...
import  com.lmt.lmtech.ads.dialog.PrepareLoadingAdsDialog;
import com.lmt.lmtech.ads.dialog.ResumeLoadingDialog;
import com.lmt.lmtech.ads.event.ExpediteeLogEventManager;
...
public class AppOpenManager implements Application.ActivityLifecycleCallbacks, LifecycleObserver {
    ...
    private String appResumeAdId2ID = null;
    ...
    public void init(Application application, String appOpenAdId2ID, String appResumeAdId) {
        isInitialized = true;
        disableAdResumeByClickAction = false;
        this.myApplication = application;
        this.myApplication.registerActivityLifecycleCallbacks(this);
        ProcessLifecycleOwner.get().getLifecycle().addObserver(this);
        this.appResumeAdId2ID = appOpenAdId2ID;
    ...
    public void fetchAd(final boolean isSplash) {
        ...
        // Logic load 2 id cho ad resume (appResumeAdId2ID ưu tiên, fail thì fallback appResumeAdId)
        if (!isSplash && appResumeAdId2ID != null) {
            Log.d(TAG, "fetchAd: loading app resume ad - primary id = " + appResumeAdId2ID + ", fallback id = " + appResumeAdId);

            if (currentActivity != null) {
                if (AppPurchase.getInstance().isPurchased(currentActivity)) return;
                List<String> testIds = Arrays.asList(currentActivity.getResources().getStringArray(R.array.list_id_test));
                if (testIds.contains(appResumeAdId2ID)) {
                    showTestIdAlert(currentActivity, false, appResumeAdId2ID);
                }
            }

            AdRequest request = getAdRequest();
            Log.d(TAG, "fetchAd: start load app open ad with id = " + appResumeAdId2ID);
            AppOpenAd.load(
                    myApplication,
                    appResumeAdId2ID,
                    request,
                    new AppOpenAd.AppOpenAdLoadCallback() {
                        ...
                        public void onAdLoaded(@NonNull AppOpenAd ad) {
                            Log.d(TAG, "onAppOpenAdLoaded (primary id2) success, adUnitId = " + ad.getAdUnitId());
                            AppOpenManager.this.appResumeAd = ad;
                            AppOpenManager.this.appResumeAd.setOnPaidEventListener(adValue -> {
                                ExpediteeLogEventManager.logPaidAdImpression(myApplication.getApplicationContext(),
                                        adValue,
                                        ad.getAdUnitId(),
                                        ad.getResponseInfo()
                        ...
                        public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                            Log.d(TAG, "onAppOpenAdFailedToLoad primary id = " + appResumeAdId2ID + ", message: " + loadAdError.getMessage() + ", try fallback id = " + appResumeAdId);

                            // Fallback sang appResumeAdId nếu có
                            ...
                            if (currentActivity != null) {
                                List<String> testIds = Arrays.asList(currentActivity.getResources().getStringArray(R.array.list_id_test));
                                if (testIds.contains(appResumeAdId)) {
                                    showTestIdAlert(currentActivity, false, appResumeAdId);
                                }
                            }

                            AdRequest fallbackRequest = getAdRequest();
                            Log.d(TAG, "fetchAd: start load app open ad (fallback) with id = " + appResumeAdId);
                            AppOpenAd.load(myApplication, appResumeAdId, fallbackRequest, new AppOpenAd.AppOpenAdLoadCallback() {
                                        @Override
                                        public void onAdLoaded(@NonNull AppOpenAd ad) {
                                            Log.d(TAG, "onAppOpenAdLoaded (fallback id1) success, adUnitId = " + ad.getAdUnitId());
                                            AppOpenManager.this.appResumeAd = ad;
                                            AppOpenManager.this.appResumeAd.setOnPaidEventListener(adValue -> {
                                                ExpediteeLogEventManager.logPaidAdImpression(myApplication.getApplicationContext(),
                                                        adValue,
                                        ...
                                        @Override
                                        public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                                            Log.d(TAG, "onAppOpenAdFailedToLoad fallback id = " + appResumeAdId + ", message: " + loadAdError.getMessage());
                                        }
                                    }
                            );
                        }
        ...
        // Logic cũ cho splash hoặc trường hợp chỉ có 1 id resume
        loadCallback = new AppOpenAd.AppOpenAdLoadCallback() {
            @Override
            public void onAdLoaded(AppOpenAd ad) {
                Log.d(TAG, "onAppOpenAdLoaded success, isSplash = " + isSplash + ", adUnitId = " + ad.getAdUnitId());
                if (!isSplash) {
                    AppOpenManager.this.appResumeAd = ad;
                    AppOpenManager.this.appResumeAd.setOnPaidEventListener(adValue -> {
                        ExpediteeLogEventManager.logPaidAdImpression(myApplication.getApplicationContext(),
            ...
            @Override
            public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                Log.d(TAG, "onAppOpenAdFailedToLoad: isSplash = " + isSplash + ", id = " + (isSplash ? splashAdId : appResumeAdId) + ", message = " + loadAdError.getMessage());
            }
        };
        String adIdToLoad = isSplash ? splashAdId : appResumeAdId;
        Log.d(TAG, "fetchAd: loading " + (isSplash ? "splash" : "app resume") + " ad with id = " + adIdToLoad);

        if (currentActivity != null) {
            if (AppPurchase.getInstance().isPurchased(currentActivity)) return;
            if (Arrays.asList(currentActivity.getResources().getStringArray(R.array.list_id_test)).contains(adIdToLoad)) {
                showTestIdAlert(currentActivity, isSplash, adIdToLoad);
            }

        }
        AdRequest request = getAdRequest();
        Log.d(TAG, "fetchAd: start load app open ad with id = " + adIdToLoad);
        AppOpenAd.load(myApplication, adIdToLoad, request, loadCallback);
    }
                    ...
                    splashAd.setFullScreenContentCallback(
                            new FullScreenContentCallback() {
                                ...
                                public void onAdClicked() {
                                    super.onAdClicked();
                                    if (currentActivity != null) {
                                        ExpediteeLogEventManager.logClickAdsEvent(currentActivity, splashAdId);
...
//                        fetchAd(false);
                    ...
                    public void onAdClicked() {
                        super.onAdClicked();
                        if (currentActivity != null) {
                            ExpediteeLogEventManager.logClickAdsEvent(currentActivity, appResumeAdId);
                            if (fullScreenContentCallback != null) {
                                fullScreenContentCallback.onAdClicked();
                            }
                        }
                ...
                new InterstitialAdLoadCallback() {
                    ...
                    public void onAdLoaded(@NonNull InterstitialAd interstitialAd) {
                        ...
                        interstitialAd.setOnPaidEventListener(adValue -> {
                            ExpediteeLogEventManager.logPaidAdImpression(activity,
                                    adValue,
        ...
        new Handler().postDelayed(() -> {
            splashAd.setFullScreenContentCallback(
                    new FullScreenContentCallback() {
                        ...
                        public void onAdClicked() {
                            super.onAdClicked();
                            ExpediteeLogEventManager.logClickAdsEvent(context, splashAdId);
    ...
    public void onStop() {
        Log.d(TAG, "onStop: app stop " + currentActivity);
        if (splashActivity == null) {
            if (currentActivity == null) return;
            if (!currentActivity.getClass().getName().equals(AdActivity.class.getName())) {
                Log.d(TAG, "onStop 1: with " + currentActivity.getClass().getName());
                fetchAd(false);
            }
        } else {
            if (currentActivity == null) return;
            if (!currentActivity.getClass().getName().equals(splashActivity.getName()) && !currentActivity.getClass().getName().equals(AdActivity.class.getName())) {
                Log.d(TAG, "onStop 2: with " + currentActivity.getClass().getName());
                fetchAd(false);
            }
        }
    ...
    public void loadOpenAd(Context context, String idOpenAd, AppOpenAd.AppOpenAdLoadCallback callback) {
        AdRequest adRequest = this.getAdRequest();
        AppOpenAd.AppOpenAdLoadCallback appOpenAdLoadCallback = new AppOpenAd.AppOpenAdLoadCallback() {
            public void onAdFailedToLoad(@NonNull LoadAdError loadAdError) {
                super.onAdFailedToLoad(loadAdError);
        ...
        AppOpenAd.load(context, idOpenAd, adRequest, appOpenAdLoadCallback);
    }

    public void showOpenAd(Activity activity, AppOpenAd appOpenAd, AdCallback adCallback) {
        if (appOpenAd == null) {
        ...
        appOpenAd.setFullScreenContentCallback(new FullScreenContentCallback() {
            ...
            public void onAdClicked() {
                super.onAdClicked();
                ExpediteeLogEventManager.logClickAdsEvent(activity, appOpenAd.getAdUnitId());
                adCallback.onAdClicked();
            }
        });
    ...
    public boolean isAppOpenShowed() {
